...
_declspec(dllexport) int fn_NetSim_Zigbee_Metrics(PMETRICSWRITER metricsWriter)
{
	...
	PMETRICSNODE table = init_metrics_node(MetricsNode_Table,
		"IOT Metrics for Energy Harvesting", NULL);
	NETSIM_ID nDeviceCount = NETWORK->nDeviceCount;
	add_table_heading(table, "Node Name", true, 0);
	add_table_heading(table, "Status", true, 0);
	add_table_heading(table, "Time(sec)", true, 0);
	add_table_heading(table, "Residual Energy(mJ)", true, 0);
	int i;
	for (i = 1; i <= nDeviceCount; i++)
	{
		if(DEVICE_MACLAYER(i, 1)->nMacProtocolId== MAC_PROTOCOL_IEEE802_15_4)
		NetSim_Residual_Energy[i - 1] = pstruDevicePower[i - 1]->dRemainingPower;
		else
			NetSim_Residual_Energy[i - 1] = 0;
		add_table_row_formatted(false, table, "%s,%s,%.2lf,%.3lf,",
			NETWORK->ppstruDeviceList[i - 1]->szDeviceName, NetSim_Node_state[i - 1],
