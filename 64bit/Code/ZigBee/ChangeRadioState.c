...
int fn_NetSim_Zigbee_ChangeRadioState(NETSIM_ID nDeviceId, PHY_TX_STATUS nOldState, PHY_TX_STATUS nNewState,POWER** pstruDevicePower,NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	else
	{
		...
		NetSim_Off_Time[nDeviceId - 1] = ldEventTime/1000000;
		NetSim_Node_state[nDeviceId - 1] = "OFF";
		return nStatus;

	}
}
#define EH_FRACTION 0.5
int calculate_eh(NETSIM_ID dev1,NETSIM_ID dev2)
{
	pstruDevicePower[dev2-1]->dRemainingPower += EH_FRACTION*GET_RX_POWER_mw(dev1, dev2, pstruEventDetails->dEventTime);
